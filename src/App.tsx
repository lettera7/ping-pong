...
function parseRatingsFromSheet(table: (string | number)[][]): MonthlyRecord[] {
  ...
  for (let col = 1; col < headers.length; col++) {
    ...
    records.push({ month: monthName, winner: fallback?.winner ?? standings[0][0], winnerNote: fallback?.winnerNote, standings });
  }
...
export default function App() {
  ...
  function submitMatch() {
    ...
    // Use monthly-reset ratings (from 1000) not cumulative all-time ratings
    const rA = currentMonthView.standings.find(s => s.name === pA)?.rating ?? 1000;
    const rB = currentMonthView.standings.find(s => s.name === pB)?.rating ?? 1000;
    const eA = 1 / (1 + Math.pow(10, (rB - rA) / 400));
