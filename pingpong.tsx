...
function replayMatches(rawMatches) {
  const players = {};
  const matches = [];
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    try {
      const result = await window.storage.get("pp_matches", true);
      const raw = result ? JSON.parse(result.value) : SEED_MATCHES;
      setState(replayMatches(raw));
    } catch (e) {
      setState(replayMatches(SEED_MATCHES));
    }
  }, []);
  ...
  async function saveMatch(match) {
    setSaving(true);
    // 1. Salva nello storage condiviso (per sincronizzare tra dispositivi)
    try {
      let existing = SEED_MATCHES;
      try {
        const result = await window.storage.get("pp_matches", true);
        if (result) existing = JSON.parse(result.value);
      } catch(e) {}
      const updated = [...existing, { date: match.date, playerA: match.playerA, playerB: match.playerB, scoreA: match.scoreA, scoreB: match.scoreB }];
      await window.storage.set("pp_matches", JSON.stringify(updated), true);
    } catch(e) {}

    // 2. Scrivi su Google Sheet tramite img trick (bypassa CORS)
    try {
      ...
      showFlash("Errore scrittura Excel");
    }
    setSaving(false);
  }
  ...
  function submitMatch() {
    ...
    const match = { id: Date.now(), date, playerA: pA, playerB: pB, scoreA, scoreB, winner, rA, rB, newA, newB, dA, dB: -dA };
    setState({
      players: {
        ...players,
  ...
  function addPlayer() {
    ...
    if (!name || state.players[name]) return showFlash("Nome non valido o gia esistente.");
      ...
      {view==="result" && lastResult && (
        <div style={{ padding:"24px 20px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
            {[{name:lastResult.pA,d:lastResult.dA,r:lastResult.newA},{name:lastResult.pB,d:lastResult.dB,r:lastResult.newB}].map(p=>(
...
const btn = (bg,color) => ({ display:"block", width:"100%", padding:"14px 0", background:bg, color, border:"none", fontFamily:"inherit", fontWeight:900, fontSize:12, letterSpacing:2, textTransform:"uppercase", cursor:"pointer" });
const inp = { width:"100%", background:LGRAY, border:"none", borderBottom:"2px solid #0D0D0D", color:"#0D0D0D", padding:"12px 10px", fontSize:16, fontFamily:"inherit", boxSizing:"border-box", outline:"none" };
const sel = { width:"100%", background:LGRAY, border:"none", borderBottom:"2px solid #0D0D0D", color:"#0D0D0D", padding:"12px 10px", fontSize:14, fontFamily:"inherit", appearance:"none", outline:"none", boxSizing:"border-box" };
const lbl = { fontSize:9, letterSpacing:2, color:"#888", textTransform:"uppercase", marginBottom:6 };
